...
def extract_speaker_and_message(line):
    ...
    for i in range(start_index, len(line)):
        if line[i:i+5] == "Anushka " or line[i:i+3] == "Adi":
            end_index = i
            break
    ...
    elif line.startswith("Anushka Mishra ✨"):
        speaker = "Anushka"
        message = line.replace("Anushka Mishra ✨: ", "").strip()
    return speaker, message

input_file = "D:/Python/Projects/Whatsapp AI Model/Dataset/WhatsApp Chat with Anushka Mishra ✨.txt"
data = preprocess_chat_data(input_file)

output_file = "preprocessed_data_Anushka.json"
