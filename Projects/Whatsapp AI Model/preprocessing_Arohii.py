...
def extract_speaker_and_message(line):
    ...
    for i in range(start_index, len(line)):
        if line[i:i+5] == "Arohi" or line[i:i+3] == "Adi":
            end_index = i
            break
    ...
    elif line.startswith("Arohi Lps Prsnl💙"):
        speaker = "Arohii"
        message = line.replace("Arohi Lps Prsnl💙: ", "").strip()
    return speaker, message

input_file = "D:/Python/Projects/Whatsapp AI Model/Dataset/WhatsApp Chat with Arohi Lps Prsnl💙.txt"
data = preprocess_chat_data(input_file)

output_file = "preprocessed_data_Arohii.json"
